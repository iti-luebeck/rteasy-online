...
fn register_name() {
    ...
    let vhdl = util::compile(SOURCE).render("module", false, Default::default()).unwrap();
...
fn module_name() {
    ...
    let vhdl = util::compile(SOURCE).render("my_vhdl_module", false, Default::default()).unwrap();
